...
red_pitaya_hk #(.DWE(DWE)) i_hk (
  ...
  .led_o(), .digital_loop(digital_loop), .daisy_mode_o(),
...
////////////////////////////////////////////////////////////////////////////////
// SYS [1] through SYS [7]: AXI/SYS bus test slaves
//
// Address windows:
//   sys[1] -> 0x40100000
//   sys[2] -> 0x40200000
...
//   sys[7] -> 0x40700000
//
// Do NOT instantiate sys_bus_stub for sys[1]..sys[7] when these are used.
////////////////////////////////////////////////////////////////////////////////

generate
for (genvar gi = 1; gi < 8; gi++) begin : gen_axi_test
  axi_test #(
    .ID(gi)
  ) i_axi_test (
    .clk_i     (adc_clk),
    ...
    .sys_addr  (sys[gi].addr),
    .sys_wdata (sys[gi].wdata),
    .sys_wen   (sys[gi].wen),
    .sys_ren   (sys[gi].ren),
    .sys_rdata (sys[gi].rdata),
    .sys_err   (sys[gi].err),
    .sys_ack   (sys[gi].ack)
  );
end
